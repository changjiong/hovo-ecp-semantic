...
6. 每份输入来源具有上游结构化状态和完整来源单元集合；原始文档字节可追溯，SourceUnit 文本摘要一致，提供 parser 元数据时其 input_digest 与原始 ArtifactRef 一致。每个来源单元恰好对应一条 ProvisionCoverage。`PARTIAL`、`UNREADABLE` 和 `FAILED` 必须形成显式缺口；本技能不得把“已提供单元全部处理”表述成“原始制度文件解析完整”。
