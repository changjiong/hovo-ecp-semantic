...
对外使用 [request.schema.json](request.schema.json) 1.0.0：PRODUCE/REVISE 只要求业务任务和原始 documents，用户不需要构造 SourceRef、SourceUnit 或 SourceExtraction。

Skill 内部 document-intake 调用一个已配置的外部解析服务，并把响应规范化为 [input.schema.json](input.schema.json) 4.0.0；其中 SourceRef、SourceUnit、SourceExtraction 组成 Structured Document IR。领域知识 `output.json` 的唯一 `input_ref` 继续绑定这份内部 normalized input，因此下游合同和审计逻辑不变。

解析算法由外部服务提供；本技能只负责调用、规范化、状态传播和内部合同校验，不安装 OCR/PDF/DOCX 解析库。

- PRODUCE：从原始 documents 开始，经 document-intake 形成内部结构化来源单元，再形成业务知识草案。
...
SourceRef 定位原始材料、来源主体、时间、版本、原文位置及来源角色。SourceUnit 是 document-intake 根据外部解析响应规范化形成的可核对来源单元；SourceExtraction 记录本次外部解析状态、方法、限制和完整单元集合。可选的 source_locator、sequence、parent_unit_id 与 parser 元数据用于增强定位和重放。SOURCE_STATED 陈述必须同时指向来源和具体条款单元，Term/Rule.statement_ids 指向陈述，Case.question_ids 指向业务问题。`Question` 以 `topic` 归组，可用 `parent_question_id` 表示无环父子关系；`explanation`、Term 的 `example`/`counterexample` 与 Case 的 `reasoning` 为独立业务解释的结构化载体。PRODUCE 或 REVISE 新增的判断和样章必须填写适用解释；未重审的既有范围可以缺省，不得用空白补造解释。
