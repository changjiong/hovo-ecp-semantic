...
<project>/
  ...
  reports/<run-id>/<stage>/   实际检查及交付状态
```

阶段目录是运行项目的唯一产物边界。正式输入和交付物放在对应阶段根目录；请求快照、解析中间结果、生成脚本、调试文件、验证结果和评审记录放在该阶段的 `process/` 下，并按 `runs/`、`review/`、`validation/`、`confirmation/`、`package/` 等职责继续分开。技能包自身的 `reports/` 只保存技能开发/发布证据，不接收项目运行产物。

`reports/<run-id>/<stage>/` 仅用于跨阶段或需要统一汇总的运行证据。任何中间文件不得写入技能包源码目录，也不得与正式 `output.json` 混放。历史版本保留在产生它的阶段 `archive/`，不使用旧路径兼容或回退。
