...
Hovo 0.7.2 开发候选包 · 模型合同 5.0.0 · DSL 2.0.0 · 上游知识合同 4.0.0。当前在本项目 `.agents/skills/domain-model` 本地使用，未发布或验证宿主重新发现。
...
0.7.2延续并强化：业务名称和实例要求、全量范围门槛、规则七要素覆盖、规则字段绑定、过程依赖、人工裁定准则，以及业务视图与审计视图分开生成。规则和案例引用齐全仍须语义回读，不能单凭数量宣称业务完成。

当前0.7.2沿用A01全量模型能力、结构合同检查和关键边界行为验证；86个案例逐案执行、独立业务语义评审、宿主安装和平台运行仍未证明。报告须绑定本版实际文件；缺失执行或人工评审证据的项目保留未执行。
